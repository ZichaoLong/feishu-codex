...
DATA_ROOT="$DATA_DIR"
GLOBAL_DATA_DIR="$DATA_ROOT/_global"
...
mkdir -p "$CONFIG_ROOT" "$DATA_ROOT" "$GLOBAL_DATA_DIR" "$ENV_DIR"
...
[Unit]
Description=Feishu Codex Bot (default instance)
...
Environment=FC_INSTANCE=default
Environment=FC_CONFIG_ROOT=$CONFIG_ROOT
Environment=FC_DATA_ROOT=$DATA_ROOT
Environment=FC_GLOBAL_DATA_DIR=$GLOBAL_DATA_DIR
...
EOF
    ...
    cat > "$SYSTEMD_DIR/$SERVICE_NAME@.service" << EOF
[Unit]
Description=Feishu Codex Bot (%i)
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
WorkingDirectory=%h/.local/share/$SERVICE_NAME/instances/%i
ExecStart=$VENV_DIR/bin/python -m bot
Environment=FC_INSTANCE=%i
Environment=FC_CONFIG_ROOT=$CONFIG_ROOT
Environment=FC_DATA_ROOT=$DATA_ROOT
Environment=FC_GLOBAL_DATA_DIR=$GLOBAL_DATA_DIR
Environment=FC_CONFIG_DIR=%h/.config/$SERVICE_NAME/instances/%i
Environment=FC_DATA_DIR=%h/.local/share/$SERVICE_NAME/instances/%i
EnvironmentFile=-%h/.config/environment.d/90-codex.conf
Environment=PATH=$SERVICE_PATH
Restart=on-failure
RestartSec=10
...
EOF
    ...
    _green "  ✓ 默认实例服务已注册并设为开机自启: $SYSTEMD_DIR/$SERVICE_NAME.service"
    _green "  ✓ 多实例模板服务已注册: $SYSTEMD_DIR/$SERVICE_NAME@.service"
...
_validate_instance() {
    ...
    if [[ ! "\$name" =~ ^[a-z0-9][a-z0-9._-]{0,63}$ ]]; then
        echo "[feishu-codex] 非法 instance 名称: \$name" >&2
        exit 2
    fi
}
...
if [[ "\${1:-}" == "--instance" ]]; then
    if [[ -z "\${2:-}" ]]; then
        echo "[feishu-codex] --instance 缺少实例名" >&2
        exit 2
    fi
    INSTANCE="\$2"
...
if [[ "\$INSTANCE" == "default" ]]; then
    ...
    CONFIG_DIR="$CONFIG_ROOT/instances/\$INSTANCE"
    DATA_DIR="$DATA_ROOT/instances/\$INSTANCE"
    SERVICE_UNIT="$SERVICE_NAME@\${INSTANCE}.service"
fi

_ensure_instance_layout() {
    mkdir -p "\$CONFIG_DIR" "\$DATA_DIR" "$GLOBAL_DATA_DIR"
    if [[ ! -f "\$CONFIG_DIR/system.yaml" && -f "$CONFIG_ROOT/system.yaml.example" ]]; then
        cp "$CONFIG_ROOT/system.yaml.example" "\$CONFIG_DIR/system.yaml"
    fi
    if [[ ! -f "\$CONFIG_DIR/codex.yaml" && -f "$CONFIG_ROOT/codex.yaml.example" ]]; then
        cp "$CONFIG_ROOT/codex.yaml.example" "\$CONFIG_DIR/codex.yaml"
    fi
    if [[ ! -s "\$CONFIG_DIR/init.token" ]]; then
        "$VENV_DIR/bin/python" - "\$CONFIG_DIR/init.token" <<'PY'
import os
import pathlib
import secrets
import sys
...
case "\${1:-}" in
    ...
    log)
        _needs_systemd
        journalctl --user -u "\$SERVICE_UNIT" -f --output=cat
        ;;
    run)
        echo "[feishu-codex] 前台运行模式（Ctrl+C 退出）"
        _ensure_instance_layout
        cd "$DATA_DIR"
        export FC_INSTANCE="\$INSTANCE"
        export FC_CONFIG_ROOT="$CONFIG_ROOT"
        export FC_DATA_ROOT="$DATA_ROOT"
        export FC_GLOBAL_DATA_DIR="$GLOBAL_DATA_DIR"
    ...
    uninstall)
        ...
        echo "  systemd 模板:  ~/.config/systemd/user/\$SERVICE_NAME@.service"
        ...
        echo "  数据根目录:    $DATA_ROOT  （含多实例数据与 venv）"
...
rm -f "\$HOME/.local/bin/feishu-codex"
    ...
    purge)
        ...
        echo "  管理命令:      ~/.local/bin/feishu-codex"
        echo "  本地管理 CLI:  ~/.local/bin/feishu-codexctl"
        echo "  配置根目录:    $CONFIG_ROOT  （含所有实例飞书凭证）"
        echo "  数据根目录:    $DATA_ROOT  （含所有实例数据与 venv）"
        echo ""
        echo "以下内容不会删除："
        echo "  项目源码:      $INSTALL_DIR"
        echo ""
        ...
        ;;
    *)
        echo "用法: feishu-codex [--instance <name>] {start|stop|restart|status|log|run|config|uninstall|purge}"
        echo ""
        echo "  start     启动指定实例（默认 default）"
        echo "  stop      停止指定实例"
        echo "  restart   重启指定实例"
        echo "  status    查看指定实例状态"
        echo "  log       查看指定实例实时日志（Ctrl+C 退出）"
        echo "  run       前台运行指定实例（调试用，直接执行不走 systemd）"
        echo "  config    打开指定实例配置文件（飞书凭证 / Codex 参数）"
        echo "  管理 binding 请使用: feishu-codexctl binding clear <binding_id> | clear-all"
        echo "  uninstall 全局卸载服务和管理命令（保留所有实例配置、数据、源码）"
        echo "  purge     全局彻底清除（在 uninstall 基础上额外删除所有实例配置和数据）"
        ;;
esac
SCRIPT
