...
6. 在停用前先让 `MemberA` 发起一个会产生 approval 或补充输入的请求，确认该群当前存在 pending 卡片。
7. `Admin` 发送 `@机器人 /group deactivate`。预期：停用成功，并立即 fail-close/失效该群旧 binding 的 pending interaction 与 FIFO；不等待 turn 超时或下一次重启。
8. 立即点击第 6 步留下的旧审批/补充输入卡片。预期：动作被拒绝或显示已失效，绝不能继续旧请求；重新激活群也不能让旧卡复活。
9. `MemberA` 再次发送 `@机器人 你好` 或 `/status`。预期：再次收到拒绝提示。
10. 可选：重新激活后让 `Admin` 退群。`MemberA` 再次 `@机器人 你好`。预期：群继续可用；但群里的共享状态命令仍无法由普通成员执行。
