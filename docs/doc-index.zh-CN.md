...
| 本地命令面应如何重划？`unsubscribe`、`fcodex` thin wrapper、`feishu-codexctl` 分工、thread-wise profile/provider 的目标合同是什么？ | [`local-command-and-thread-profile-contract.zh-CN.md`](./contracts/local-command-and-thread-profile-contract.zh-CN.md) |
