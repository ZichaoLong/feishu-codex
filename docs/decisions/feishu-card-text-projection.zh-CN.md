...
- 列表项首行后的缩进续行，以及列表项 / 列表项续行后紧跟 nested list 的
  换行，可以显式插入 `<br>`，避免飞书卡片把列表内 soft break 渲染成同一行；
  更深层 nested list item 后紧跟祖先列表项续行时，也可以在前一行补这个硬换行
- 如果 nested list 结束后还有最外层父级列表项续行，发送侧展示投影可以把该续行
  去掉父项缩进，使它按原始顺序显示在 nested list 后面，而不是被飞书提前拼回
  父项首行；更深层的祖先续行保留缩进。这些规则只作用于 fenced code block 外，
  不会把非列表上下文里的四空格缩进代码块当作列表项 opener
