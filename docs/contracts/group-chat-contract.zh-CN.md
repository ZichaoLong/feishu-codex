...
- 只有被有效 mention 时才回复
- 未有效 mention 的普通消息永远只作为上下文记录，不会因为当前群 binding 正在执行而变成 queued prompt
- 有效 mention 在当前群 binding idle 时立即触发；在当前群 binding 正在执行时，按同一 binding 的本地 FIFO 排队
- 回复时附带自上次触发边界以来的群上下文
...
- 当前群 binding idle 时，普通文本立即触发；当前群 binding 正在执行时，普通文本按同一 binding 的本地 FIFO 排队
- 风险最高，容易刷屏
...
- queued mention 出队时，按这条 queued mention 的原始位置执行一次历史回捞：
  - 回捞发生在出队瞬间
  - 区间起点是当前已记录的 assistant 上下文边界
  - 区间终点是 queued mention 这条消息的初始 `created_at` / `message_id` / 本地日志 `seq`
  - queued mention 自身作为当前 turn，不混入历史上下文
  - 构造完上下文并开始该 queued turn 后，assistant 边界推进到这条 queued mention
- 回捞区间按消息初始发送时间判定，不按卡片或消息最后更新时间判定
- 区间内消息内容按出队回捞时飞书可见的当前内容读取；如果外部机器人卡片在被消费后继续更新，后续有效 mention 不会因为该更新而自动回头重捞
- 回捞内容包括初始发送时间落在区间内、且不是本机器人发送的消息；本机器人自己的消息不进入 assistant 历史上下文
