...
- `/status`、`/preflight` 与 `/release-feishu-runtime` 即使在群里触发，仍按 chat-scoped 命令解释
...
`/help` 下的 `settings` 分支负责默认 profile 与当前 binding 的运行时设置。
...
群里触发的 `/status`、`/preflight`、`/release-feishu-runtime`、`/profile` 等通用 Feishu 命令，不属于 `group` 分支。
