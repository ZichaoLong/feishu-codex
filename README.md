...
- 人类成员权限：按群管理，使用 `/acl`
- ACL 只决定谁具备群聊触发资格；是否必须 `@机器人` 由群工作态决定
- 在群聊 `assistant` 和 `mention-only` 工作态下，**普通对话和群命令都需要先 `@机器人`**
...
| `im:message.group_msg` | 支持群聊 `assistant` / `all` 工作态，读取非 `@机器人` 的群消息 |
...
- 这意味着：默认只有配置里的管理员具备群聊触发资格；其他成员即使 `@机器人` 也不会触发
- 如果要让机器人读取群里所有消息，必须为应用开通 `im:message.group_msg`

群命令触发规则：

- 私聊：直接发送命令即可，例如 `/whoami`
- 群聊 `assistant` / `mention-only`：群命令也需要先 `@机器人`，例如 `@机器人 /groupmode`、`@机器人 /acl`
- 群聊 `all`：人类成员可直接发送群命令

群聊工作态：

- `mention-only`：只有 `@机器人` 的消息会触发；不缓存群上下文
- `assistant`：缓存群里收到的消息，但只有被人类 `@` 时才回复；每次有效 `@` 都会按配置回捞时间窗内缺失的最近历史消息，并把两次 `@` 之间的消息统一并入同一份上下文。上下文边界会同时记录边界时间戳下已消费的 `message_id`，用于避免同毫秒缺失消息被漏掉。回捞开始前会先出现一张“准备群聊上下文”的执行卡片；若回捞失败，则本次会直接报错停止，避免带着残缺上下文继续回复
- `all`：群里消息都会直接触发机器人回复；风险最高，容易刷屏
- 群命令不会写入 `assistant` 上下文日志，也不会推进上下文边界
- 其他机器人不会直接触发当前机器人；它们的群消息如果要进入上下文，依赖 `assistant` 在每次有效 `@` 时做历史回捞

管理员命令：

- 在 `assistant` / `mention-only` 中，下面这些群命令要用 `@机器人 /...` 发送；在 `all` 中可直接发送
- `/groupmode`：查看当前群聊工作态
- `/groupmode assistant`
- `/groupmode all`
...
5. 如果你希望回复能参考其他机器人发言，确保应用已开通 `im:message.group_msg`，并保留历史回捞配置
...
上例表示：每次有效被 `@` 时，最多补最近 6 小时内的 30 条历史消息。
