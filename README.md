...
当前状态是可本地安装（Linux / macOS / Windows）、可启动、核心链路可用的 MVP，功能仍在持续补齐中。
...
1. 安装：
   ...
   `install.py` 是唯一的安装实现；`install.sh` 和 `install.ps1` 只是按平台选择 Python 并转调它。
...
- 安装代码包与依赖
- 初始化当前平台默认配置/数据目录
- 刷新本地默认模板 `system.yaml.example` 与 `codex.yaml.example`
- 生成初始化口令文件 `init.token`
- 安装 `feishu-codex` / `feishu-codexd` / `feishu-codexctl` / `fcodex` 本地命令包装器
- 后续由 `feishu-codex start` 按平台接入对应的用户态 service manager：
  Linux `systemd --user`
  macOS `LaunchAgent`
...
- 管理员、群 ACL、群触发身份都按 `open_id` 工作；先私聊机器人发 `/whoami` 拿到自己的 `open_id`
- 群聊显式 mention 触发依赖 `bot_open_id`；可先私聊机器人发 `/whoareyou` 辅助探测，再写回 `system.yaml`
- 群里的所有 `/` 命令都只给管理员；在 `assistant` / `mention-only` 中，管理员命令和普通对话都仍需要先显式 mention 触发对象
...
- 在群聊 `assistant` 工作态下，每次有效人类 `@` 都会额外回捞最近群历史
...
```bash
feishu-codex config codex --open
```

如果 provider key 走环境变量，推荐统一放在：

```ini
# feishu-codex config env
provider1_api_key=...
provider2_api_key=...
```
...
- 严格的群聊身份、mention、`trigger_open_ids` 语义见 `docs/contracts/group-chat-contract.zh-CN.md`
...
## 启动与使用
统一使用 `feishu-codex` 管理服务。它会按平台分别接到 `systemd --user`、`LaunchAgent`、`Task Scheduler`：

```bash
feishu-codex start/stop/restart/status # 默认实例
feishu-codex --instance corp-a start|stop|restart|status
feishu-codex log # 默认实例日志
feishu-codex --instance corp-a log
feishu-codex run # 默认实例前台调试
feishu-codex --instance corp-a run
feishu-codex config/uninstall/purge # 其他便捷命令: 配置、卸载、卸载+删除配置
```

## 三个入口怎么分

- `feishu-codex`：本地安装与 service 管理入口；负责 install / start / stop / restart / status / log / config
- `feishu-codexd`：真正的后台 daemon 入口；一般由 `feishu-codex start` 间接启动
- 飞书聊天命令：面向“当前 chat binding”，处理当前会话的提问、恢复、设置、群聊管理与当前线程状态，例如 `/help`、`/session`、`/status`、`/preflight`
- `fcodex`：面向 shared backend 的本地 Codex 使用入口；适合在本地继续飞书正在操作的 live thread，或按与飞书一致的发现规则恢复线程；多实例时支持 `--instance <name>`
- `feishu-codexctl`：面向“运行中的 `feishu-codex` 服务”的本地管理 CLI；适合看全局 binding / thread 状态、管理实例 admission、按 thread 释放 Feishu runtime；多实例时支持 `--instance <name>`
...
- 命名实例默认只暴露显式导入（admitted）的 thread；default 实例继续沿用原单实例全局可见行为
...
- `/session`：查看当前目录线程
- `/resume <thread_id|thread_name>`：恢复线程，并切换到线程自己的目录；命名实例只允许恢复已导入或当前已绑定的 thread
- `/new`：立即新建线程
- `/cd <path>`、`/pwd`、`/status`、`/preflight`、`/cancel`
- `/rename <title>`、`/rm [thread_id|thread_name]`
- `/profile`：查看或切换当前实例的 feishu-codex 默认 profile
...
- `fcodex --instance <name>`：显式连到某个实例 backend
- `fcodex /help`
- `fcodex /session [global]`
- `fcodex /resume <thread_id|thread_name>`
- `fcodex --dry-run /session [global]`
- `fcodex --dry-run /resume <thread_id|thread_name>`
- `fcodex /rm <thread_id|thread_name>`
- `fcodex /profile [name]`

如果你需要在本地继续飞书相关的会话，见下文“什么时候用 `fcodex`”。
这些命令的精确匹配、恢复、profile 语义见 `docs/contracts/session-profile-semantics.zh-CN.md`。
多实例下还要记住一条：

- 飞书 `/session`、飞书 `/resume` 受当前实例 admission 约束
- `fcodex /session`、`fcodex /resume <name>` 是本地操作者视角，不读取该 admission 过滤

本地 `feishu-codexctl`：
`feishu-codexctl` 是运行中 Feishu 服务的本地管理 CLI，不走飞书聊天命令面，也不是 `fcodex` 的别名。

- `feishu-codexctl service status`：查看 service、control endpoint、app-server 地址与整体 binding/thread 概览
...
- `feishu-codexctl thread import --thread-name <name>`
- `feishu-codexctl thread revoke --thread-id <id>`
- `feishu-codexctl thread revoke --thread-name <name>`

说明：

- thread 目标必须显式写成 `--thread-id` 或 `--thread-name`，不会再猜输入到底是 id 还是 name
- 多实例下可在最前面加 `--instance <name>`，否则默认走 current/default；歧义时会要求显式指定
- 开发期如果你要把飞书侧整体重置回“重新选择 thread”的状态，优先使用 `binding clear` / `clear-all`
- 精确的 thread-name 解析和 runtime 控制面合同见 `docs/contracts/runtime-control-surface.zh-CN.md`

## 什么时候用 `fcodex`

- 要在本地继续飞书正在写的同一线程时，用 `fcodex`。
- 要先按与飞书共享的分页 / 精确匹配算法查找线程、确认 `thread_id` 时，用 `fcodex /session [global]`。
- 命名实例下，如果你要从本地管理员视角查找所有 persisted thread，也优先用 `fcodex`；飞书侧仍会受 admission 约束。
- 只是开一个独立本地会话时，直接用裸 `codex`。

如果你已经拿到精确 `thread_id`，也可以用 `fcodex /resume <thread_id>` 直接恢复。更完整的 `session` / `resume`、profile / provider 恢复边界见 `docs/contracts/session-profile-semantics.zh-CN.md`
...
- `/profile` 改的是当前实例下 feishu-codex / 默认 `fcodex` 的本地默认 profile，不改裸 `codex` 全局配置

## 按问题查文档

如果你遇到的是具体使用问题，可以先按症状定位：

| 你在问什么 | 先看哪里 |
| --- | --- |
| “我能不能继续上次的对话？” | `binding` / `thread` 语义：`docs/contracts/session-profile-semantics.zh-CN.md` |
| “为什么我刚发的消息没有开始执行？” | `/status`、`feishu runtime`、`interaction owner`：`docs/contracts/runtime-control-surface.zh-CN.md` |
| “为什么 `/release-feishu-runtime` 失败？” | `docs/contracts/runtime-control-surface.zh-CN.md` 的 `/release-feishu-runtime` 合同 |
| “为什么 `/resume` 找不到线程？” | instance visibility / thread admission：`docs/contracts/runtime-control-surface.zh-CN.md` |
| “为什么本地 `fcodex` 接不上或恢复失败？” | shared backend / runtime lease：`docs/architecture/fcodex-shared-backend-runtime.zh-CN.md` |
| “为什么群聊里 assistant 模式没带上我预期的上下文？” | 群历史回捞与权限边界：`docs/contracts/group-chat-contract.zh-CN.md` |
| “为什么权限、审批、沙箱的行为和我想的不一样？” | `docs/contracts/codex-permissions-model.zh-CN.md` |

如果你已经知道自己要找的是哪一类 runtime / session 问题，再看下一节的详细索引。
...
- 想理解 `/status`、`/preflight`、`/release-feishu-runtime`、`feishu-codexctl` 这组共享状态词汇与控制面边界：`docs/contracts/runtime-control-surface.zh-CN.md`
