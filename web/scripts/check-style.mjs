#!/usr/bin/env node
// check-style.mjs — Focus Web style anti-pattern guard.
//
// Scans src/** for these retained rules:
//   no-gradient-text, no-glassmorphism, no-color-glow,
...
//   no-hardcoded-hex (DiffLines domain colors + var() fallbacks exempt),
//   no-hardcoded-font (token definitions exempt),
...
function checkFile(abs) {
  ...
  for (const { text, baseLine } of blocks) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // no-glassmorphism
      if (/backdrop-filter\s*:/i.test(raw)) {
