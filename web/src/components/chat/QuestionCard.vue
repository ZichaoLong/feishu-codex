...
import type { QuestionAnswer, QuestionResponse, UIQuestion } from '../../types';
...
const props = defineProps<{
  ...
  /** False when this browser lost its live WebSocket. The question remains
   * readable, but its answer must not be sent through stale HTTP. */
...
function isQuestionAnswered(qid: string): boolean {
  const question = props.question.questions.find((item) => item.id === qid);
  const a = answers.value[qid];
  if (!a) return question?.required === false;
...
const actionsDisabled = computed(() => busy.value || interactionDisabled.value);
const nowMs = ref(Date.now());
let countdownTimer: ReturnType<typeof setInterval> | null = null;

const autoResolutionVisible = computed(() => (
  typeof props.question.autoResolutionVisibleAtMs === 'number'
  && typeof props.question.autoResolutionDueAtMs === 'number'
  && nowMs.value >= props.question.autoResolutionVisibleAtMs
));

const autoResolutionRemaining = computed(() => {
  const dueAt = props.question.autoResolutionDueAtMs;
  if (!autoResolutionVisible.value || typeof dueAt !== 'number') return '';
...
function submit(): void {
  if (actionsDisabled.value || !canSubmit()) return;
...
<template>
  <Card class="qcard" :class="{ minimized, 'interaction-disabled': interactionDisabled }">
      ...
      <div class="qbody">
        ...
        <div class="qopts">
          <label
            ...
            :key="opt.id"
            class="qopt"
            :class="{ selected: isSelected(current.id, opt.id), disabled: interactionDisabled }"
          ...
          <label
            ...
            class="qopt"
            :class="{ selected: isOtherSelected(current.id), disabled: interactionDisabled }"
    ...
    <template v-if="!minimized" #foot>
      <div class="qfoot">
        <span v-if="autoResolutionVisible" class="qauto">
          {{ t('question.autoResolveIn', { remaining: autoResolutionRemaining }) }}
        </span>
        <Button
          v-if="step < total - 1"
          ...
          size="sm"
          variant="primary"
          :disabled="interactionDisabled || !isCurrentAnswered()"
          @click="goNext"
        ...
        <Button
          ...
          size="sm"
          variant="primary"
          :disabled="actionsDisabled || !canSubmit()"
          :loading="submitting"
          @click="submit"
        ...
        <Button
          ...
          size="sm"
          variant="secondary"
          :disabled="step === 0 || actionsDisabled"
          @click="goBack"
        >{{ t('question.back') }}</Button>
        <Button class="qfoot-btn" size="sm" variant="ghost" :loading="dismissing" :disabled="actionsDisabled" @click="dismiss">{{ t('question.dismiss') }}</Button>
      </div>
    </template>
  </Card>
</template>
...
.qstep-dot:disabled { cursor: not-allowed; opacity: 0.5; }
.qstep-dot.active {
...
.qopt.disabled { cursor: not-allowed; opacity: 0.6; }
.qopt.disabled:hover { background: transparent; }
