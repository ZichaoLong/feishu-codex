...
const props = defineProps<{
  ...
  busy?: boolean;
  /** False when this browser lost its live WebSocket. The card remains
   * observable, but it must not send a stale HTTP decision. */
...
const exactActions = computed(() => props.actions ?? []);
const interactionDisabled = computed(() => props.interactionEnabled === false);
const actionsDisabled = computed(() => Boolean(props.busy) || interactionDisabled.value);
...
function decideExact(action: { id: string; label: string }): void {
  const decision: ApprovalDecision = action.id === 'cancel'
    ? 'cancelled'
    : action.id === 'reject' ? 'rejected' : 'approved';
  act(`exact:${action.id}`, { decision, actionId: action.id, selectedLabel: action.label });
}
...
function handleKeydown(e: KeyboardEvent): void {
  ...
  if (exactActions.value.length > 0) {
    const index = Number.parseInt(e.key, 10) - 1;
    const action = exactActions.value[index];
    if (action) {
      e.preventDefault();
      decideExact(action);
    }
  ...
  <Card class="appr" :class="{ minimized }">
    ...
    <template v-if="!minimized" #foot>
      ...
      <div v-if="planReview" class="plan-actions">
        ...
        <Button v-else class="kbtn" size="sm" variant="primary" :loading="pendingAction === 'approvePlan'" :disabled="actionsDisabled" @click="approvePlan">{{ t('approval.approvePlan') }}<Kbd class="k" :keys="['1']" /></Button>
        <Button class="kbtn" size="sm" variant="secondary" :disabled="actionsDisabled" @click="revisePlan">{{ t('approval.revise') }}<Kbd v-if="planReview.options.length === 0" class="k" :keys="['2']" /></Button>
        <Button class="kbtn" size="sm" variant="danger-soft" :loading="pendingAction === 'rejectAndExit'" :disabled="actionsDisabled" @click="rejectAndExitPlan">{{ t('approval.rejectAndExit') }}<Kbd v-if="planReview.options.length === 0" class="k" :keys="['3']" /></Button>
      </div>

      <div v-else-if="exactActions.length > 0" class="abtn">
        <Button
          v-for="(action, index) in exactActions"
          :key="action.id"
          class="kbtn"
          size="sm"
          :variant="action.style === 'primary' ? 'primary' : action.style === 'danger' ? 'danger-soft' : 'secondary'"
          :loading="pendingAction === `exact:${action.id}`"
          :disabled="actionsDisabled"
          @click="decideExact(action)"
        >{{ action.label }}<Kbd v-if="index < 9" class="k" :keys="[String(index + 1)]" /></Button>
      </div>

      <!-- default actions row -->
      <div v-else class="abtn">
        <Button class="kbtn" size="sm" variant="primary" :loading="pendingAction === 'approve'" :disabled="actionsDisabled" @click="approve">{{ t('approval.approve') }}<Kbd class="k" :keys="['1']" /></Button>
        <Button class="kbtn" size="sm" variant="secondary" :loading="pendingAction === 'approveSession'" :disabled="actionsDisabled" @click="approveSession">{{ t('approval.approveSession') }}<Kbd class="k" :keys="['2']" /></Button>
        <Button class="kbtn" size="sm" variant="secondary" :loading="pendingAction === 'reject'" :disabled="actionsDisabled" @click="reject">{{ t('approval.reject') }}<Kbd class="k" :keys="['3']" /></Button>
        <Button class="kbtn" size="sm" variant="secondary" :disabled="actionsDisabled" @click="openFeedback">{{ t('approval.feedback') }}<Kbd class="k" :keys="['4']" /></Button>
      </div>
    </template>
  </Card>
</template>
