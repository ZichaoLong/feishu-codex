...
} from '../../composables/useAttachmentUpload';
...
const props = withDefaults(defineProps<{
  running?: boolean;
  /** Show Stop when current-turn identity is unknown but interruption is still allowed. */
  ...
  sessionId?: string;
  /** Confirmed writer-scope readiness; pending navigation keeps the draft mounted but inert. */
  composerReady?: boolean;
  searchFiles?: (q: string) => Promise<FileItem[]>;
  /** If undefined, attach button is hidden and paste/drag are no-ops. */
  uploadImage?: (file: Blob, name?: string) => Promise<{ fileId: string; name: string; mediaType: string } | null>;
  /** Optional authenticated loader for reopening an uploaded attachment. */
  downloadFile?: (fileId: string) => Promise<Blob>;
  /** ConversationPane-owned attachment state shared by empty/docked Composer
   *  instances. Standalone Kimi-derived uses may omit it and own one locally. */
  ...
  models?: AppModel[];
  /** Optional product-owned notice for the model/effort controls. */
...
function toggleExpand(): void {
  ...
  void nextTick(() => {
    ...
    // activate it again instead of reaching the submit boundary.
...
// ---------------------------------------------------------------------------
const attachmentUpload = props.attachmentUpload ?? useAttachmentUpload({
...
function handleInput(): void {
  ...
  if (capabilities.value.commands) updateSlashMenu();
...
// (which reads the attachments to build the payload) and the `hasUpload`
// toolbar flag.
// ---------------------------------------------------------------------------
const {
  attachments,
...
const submissionRevision = useComposerSubmissionRevision({
  ...
  attachments,
  sessionId: () => props.sessionId,
  composerReady: () => props.composerReady,
});
...
// Build the wire payload; non-image attachments remain ordinary Codex files.
...
// Only images have a browser preview; Codex interprets generic-file metadata.
...
function handleSubmit(): void {
  ...
  // Keep text and chips until every attachment can join the same submission.
  ...
  // Record commands and prompts for recall; image-only sends remain absent.
  ...
  if (trimmed && capabilities.value.commands) {
  ...
  const sourceRevision = submissionRevision.capture();
  submissionPending.value = true;
  const submission = createComposerSubmission(
    payload,
    () => submissionOwnerMounted
      && submissionRevision.isCurrent(sourceRevision),
    () => {
  ...
  if (!props.deferSubmitClear) submission.commit();
...
const sendDisabled = computed(() => (
  !props.composerReady
  || !capabilities.value.submit
  || props.starting
  || submissionPending.value
  || (props.running && !capabilities.value.submitWhileRunning)
));
...
const hasContextUsage = computed(() => props.status?.ctxRemainingPct !== null
  && props.status?.ctxRemainingPct !== undefined);

// The Focus-owned view projection follows Codex `/status`; this component only
// converts its remaining percentage to the used percentage drawn by the ring.
const pct = computed(() => {
  if (!hasContextUsage.value) return 0;
  return Math.min(100, Math.max(0, 100 - (props.status?.ctxRemainingPct ?? 100)));
});

const ctxTooltip = computed(() => {
  const used = formatTokens(props.status?.ctxUsed ?? 0);
  const max = formatTokens(props.status?.ctxMax ?? 0);
  return t('status.ctxTooltip', {
    used,
    max,
    remaining: props.status?.ctxRemainingPct ?? 0,
  });
});
  ...
  >
    ...
    <div v-if="attachments.length > 0" class="att-strip">
      <AttachmentChip
        ...
        :url="att.previewUrl"
        :file-id="att.fileId"
        :load-blob="downloadFile"
        :media-type="att.mediaType"
        :size="att.size"
        :uploading="att.uploading"
        :error="att.error"
        :activatable="att.kind === 'image' && !!att.previewUrl"
        removable
    ...
    <div v-if="previewAttachment" class="att-lightbox" @click.self="closeAttachmentPreview">
      <div class="att-lightbox-card">
        ...
        <img class="att-lightbox-media" :src="previewAttachment.previewUrl" :alt="previewAttachment.name" />
    ...
    <div class="composer-card">
      ...
      <div ref="toolbarRef" class="toolbar">
        ...
        <div class="toolbar-left">
          ...
          <span
            v-if="status && capabilities.permissions"
            class="perm-pill"
          ...
          <div
            v-if="capabilities.permissions && permDropdownOpen && status"
            class="perm-dropdown"
          ...
          <div v-if="status && capabilities.modes" ref="modesRef" class="modes">
            <button
              type="button"
              class="mode-pill"
        ...
        <div class="toolbar-right">
          ...
          <Tooltip :text="ctxTooltip">
            <span
              v-if="status && !hideContext && hasContextUsage"
              class="ctx-group"
              role="img"
              tabindex="0"
          ...
          <span
            v-if="status && capabilities.model"
            class="model-pill"
          ...
          <Tooltip v-if="(running || starting || interruptEnabled) && capabilities.interrupt" :text="t('composer.interruptTitle')">
            <button
              class="stop"
...
/* Send button — circular accent icon. Always enters the one submit boundary;
   the parent classifies idle start or exact active-turn steer. */
.send {
