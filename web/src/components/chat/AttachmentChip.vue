...
<!-- One attachment rendered as a pill chip — the SAME component for the
     ...
     bubble. Context differences are props, not restyled variants:
       ...
       - bubble:   plain chip; only controlled images can open a preview
     Tile rule: images show a real thumbnail; all other attachments use a
...
import type { AuthenticatedMediaLoader } from '../../lib/authenticatedMedia';
...
const props = withDefaults(
  defineProps<{
    ...
    fileId?: string;
    /** Optional product-specific authenticated loader for image thumbnails. */
    ...
    removable?: boolean;
    /** False renders a non-interactive metadata chip (no preview/download). */
    activatable?: boolean;
    /** Accessible label for the remove button. */
    removeLabel?: string;
  }>(),
  { uploading: false, error: false, removable: false, activatable: true },
);

const emit = defineEmits<{
  /** Primary action (for example, preview media) — the parent decides. */
  activate: [];
  remove: [];
}>();
...
<template>
  ...
  >
    <component
      ...
      class="att-activate"
      :class="{ 'is-static': !activatable }"
      :aria-label="activatable ? title : undefined"
