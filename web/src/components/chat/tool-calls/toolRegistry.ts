...
export function resolveToolRenderer(tool: ToolCall): ToolRenderer {
  if (tool.inspectionLocator?.kind === 'fileChange') return EditTool;
  if (tool.inspectionLocator?.kind === 'commandExecution') return GenericTool;
  if (tool.media && tool.status === 'ok') return MediaTool;
  const name = normalizeToolName(tool.name);
  if (name === 'edit' || name === 'write' || name === 'delete' || name === 'multi_edit') return EditTool;
