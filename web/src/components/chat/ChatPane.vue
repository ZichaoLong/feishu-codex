...
import type { ChatTurn, ApprovalBlock, FilePreviewRequest, ToolCall as ToolCallData, ToolMedia, QueuedPromptView, TurnAttachment } from '../../types';
...
const props = withDefaults(
  defineProps<{
     ...
     * transcript and gates copying the last message into the composer.
     */
    working?: boolean;
...
const emit = defineEmits<{
  ...
  openToolDiff: [tool: ToolCallData];
...
// Id of the most recent user turn — only that message offers a compact
// "copy into composer" affordance. It never alters the persisted transcript.
...
/** Whether to offer a follow-up draft from this message: latest user message,
    only while the conversation has nothing unfinished and it isn't a slash activation. */
...
function userTurnHasPresentation(turn: ChatTurn): boolean {
...
function copyMessageToComposer(turn: ChatTurn): void {
...
function userAttachmentMedia(att: TurnAttachment): ToolMedia {
  // The only user-attachment media path is a verified image. When a fileId is
  // present the preview fetches its bytes with auth (a bare URL may 401).
  return { kind: 'image', url: att.url, path: att.name, fileId: att.fileId };
}
  ...
  <div class="chat">
    ...
    <template v-for="(turn, ti) in turns" :key="turn.id">
      ...
      <template v-if="turn.role === 'user'">
          ...
          <div v-if="turn.createdAt || canCopyMessageToComposer(turn)" class="u-meta">
            <div v-if="canCopyMessageToComposer(turn)" class="u-edit-wrap">
              <button
                type="button"
                class="u-edit"
                :aria-label="t('conversation.copyToComposerTooltip')"
                @click="copyMessageToComposer(turn)"
              >
        ...
        <div
          v-else
          class="turn-anchor empty-user-anchor"
        ...
        <div class="u-bub q-bub">
          ...
          <div v-if="hasAttachments(item)" class="q-imgs">
            <template v-for="(att, ai) in item.attachments" :key="ai">
              <span v-if="att.kind !== 'image'" class="q-file">
                <Icon name="file" size="sm" />
                {{ att.name ?? att.fileId }}
              </span>
              <AuthMedia
...
/* Copy button — icon-only, shares the message action muted→hover style. */
.u-copy {
  display: inline-flex;
  align-items: center;
  justify-content: center;
...
/* Copy button — icon-only, shares the message action muted→hover style so the
   message-stream controls read as one family. */
.a-cpbtn {
  display: inline-flex;
  align-items: center;
  justify-content: center;
...
.assistant-reply-separator {
  width: 100%;
  height: 2px;
  margin-top: var(--chat-section-gap);
