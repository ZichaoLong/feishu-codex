...
import type { SessionActionCapabilities } from '../../types';
...
const actionCapabilities = computed<SessionActionCapabilities>(() => ({
  rename: props.sessionActionCapabilities?.rename ?? true,
  archive: props.sessionActionCapabilities?.archive ?? true,
  fork: props.sessionActionCapabilities?.fork ?? true,
  export: props.sessionActionCapabilities?.export ?? true,
  review: props.sessionActionCapabilities?.review ?? false,
  goal: props.sessionActionCapabilities?.goal ?? false,
}));
...
function startReview(): void {
  if (!props.sessionId) return;
  closeMenu();
  emit('reviewSession', props.sessionId);
}

function setGoal(): void {
  if (!props.sessionId) return;
  closeMenu();
  emit('goalSession', props.sessionId);
}
</script>
    ...
    >
      ...
      <template v-if="sessionId">
        ...
        <template v-if="sessionActions !== false">
          ...
          <MenuItem v-if="actionCapabilities.goal" @click="setGoal">
            <Icon name="target" size="sm" />
            {{ t('header.setGoal') }}
          </MenuItem>
          <MenuItem v-if="actionCapabilities.review" @click="startReview">
            <Icon name="search" size="sm" />
            {{ t('header.reviewSession') }}
          </MenuItem>
          <MenuItem v-if="actionCapabilities.fork" @click="forkSession">
