...
<!-- Modal overlay for choosing a model on the owning product surface. -->
...
const props = defineProps<{
  ...
  unavailable?: boolean;
  /** Optional product-owned explanation of when this selection takes effect. */
  settingsHint?: string;
}>();
  ...
  <Dialog :open="true" :close-on-esc="false" :title="t('model.title')" size="xl" height="fixed" @close="emit('close')">
    <div ref="dialogRef" class="mp">
      ...
      <div class="footer-hint">
        <div v-if="settingsHint" class="settings-hint">{{ settingsHint }}</div>
        <div>{{ t('model.footerHint') }}</div>
      </div>
    </div>
  </Dialog>
</template>
