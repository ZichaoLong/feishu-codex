...
const actionCapabilities = computed<SessionActionCapabilities>(() => ({
  rename: props.session.actionCapabilities?.rename ?? props.sessionActionCapabilities?.rename ?? true,
  archive: props.session.actionCapabilities?.archive ?? props.sessionActionCapabilities?.archive ?? true,
  fork: props.session.actionCapabilities?.fork ?? props.sessionActionCapabilities?.fork ?? true,
  export: props.session.actionCapabilities?.export ?? props.sessionActionCapabilities?.export ?? true,
  review: props.session.actionCapabilities?.review ?? props.sessionActionCapabilities?.review ?? false,
  goal: props.session.actionCapabilities?.goal ?? props.sessionActionCapabilities?.goal ?? false,
}));
...
// string like "2h"/"1d" supplied by the active client view projection).
...
const runtimeLabel = computed(() => {
  if (props.session.runtimeState === 'current') return t('focus.loadedHere');
  if (props.session.runtimeState === 'other') {
    return t('focus.loadedElsewhere', { instance: props.session.runtimeInstance || t('focus.unknown') });
  }
  if (props.session.runtimeState === 'unknown') return t('focus.loadedStateUnknown');
  if (props.session.runtimeState === 'unloaded') return t('focus.notLoaded');
  return '';
});
const selectable = computed(() => props.session.selectable !== false);
const unavailableLabel = computed(() => props.session.unavailableReason || runtimeLabel.value);

function selectRow(): void {
  if (selectable.value) emit('select', props.session.id);
}
...
<template>
  <div
    class="se"
    :class="{ on: active, unavailable: !selectable }"
    :aria-disabled="!selectable"
    :title="!selectable ? unavailableLabel : undefined"
    @click="selectRow"
  >
    <div class="row">
      ...
      <div class="left">
        ...
        <span
          v-else
          class="t"
          @dblclick.stop="sessionActions !== false && actionCapabilities.rename && startRename()"
        >{{ session.title }}</span>
      </div>
      ...
      <Tooltip v-if="runtimeLabel" :text="runtimeLabel">
        <Badge class="runtime-badge" variant="neutral" size="sm">
          {{ runtimeLabel }}
        </Badge>
      </Tooltip>
    ...
    <Teleport to="body">
      <Menu ref="menuRef" v-if="menuOpen" class="menu" :style="menuStyle" @click.stop>
        ...
        <template v-if="sessionActions !== false">
          <MenuItem separator />
          <MenuItem v-if="actionCapabilities.rename" @click="startRename">
            <Icon name="pencil" size="sm" />
            {{ t('sidebar.rename') }}
          </MenuItem>
          <MenuItem v-if="actionCapabilities.fork" @click="forkRow">
