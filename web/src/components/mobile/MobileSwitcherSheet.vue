...
function runtimeLabel(session: Session): string {
  if (session.runtimeState === 'current') return t('focus.loadedHere');
  if (session.runtimeState === 'other') {
    return t('focus.loadedElsewhere', { instance: session.runtimeInstance || t('focus.unknown') });
  }
  if (session.runtimeState === 'unknown') return t('focus.loadedStateUnknown');
  return session.runtimeState === 'unloaded' ? t('focus.notLoaded') : '';
}
...
function onArchive(id: string): void {
  menuFor.value = null;
  // The parent Focus shell owns confirmation and the async archive mutation.
  emit('archive', id);
}
...
<template>
  ...
  >
    ...
    <button v-if="allowCreate" type="button" class="newrow" @click="onCreate">
      <Icon name="message" size="sm" />
      {{ t('sidebar.newChat') }}
    </button>
    <button v-if="allowWorkspaceCreate" type="button" class="newrow secondary" @click="onAddWorkspace">
      ...
      <div v-for="g in groups" :key="g.workspace.id" class="mgroup">
        ...
        <div v-show="!isCollapsed(g.workspace.id)">
          ...
          <div
            ...
            :key="s.id"
            class="srow"
            :class="{ cur: s.id === activeId, unavailable: s.selectable === false }"
            :aria-disabled="s.selectable === false"
            :title="s.selectable === false ? s.unavailableReason : undefined"
            @click="s.selectable !== false && onSelectSession(s.id)"
          >
            <div class="m">
              ...
              <div class="s">
                <span>{{ s.time }}</span>
                <span v-if="runtimeLabel(s)"> · {{ runtimeLabel(s) }}</span>
              </div>
            </div>
            <span v-if="(attentionBySession[s.id] ?? 0) > 0" class="att">{{ attentionBySession[s.id] }}</span>
            <IconButton
              v-if="allowSessionActions && s.selectable !== false && hasSessionActions(s)"
              size="lg"
              class="kb"
              :label="t('sidebar.options')"
...
.srow.unavailable { cursor: not-allowed; opacity: 0.68; }
.srow.unavailable:hover,
.srow.unavailable:active { background: transparent; }
