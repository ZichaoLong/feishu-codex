...
const props = withDefaults(defineProps<{
  ...
  confirmLabel?: string;
  cancelLabel?: string;
  /** Require the user to type this exact value before confirming. */
...
const requiresTypedConfirmation = () => Boolean(props.confirmationText);
const typedConfirmationMatches = () => (
  !requiresTypedConfirmation() || confirmationValue.value === props.confirmationText
);

watch(
  () => [props.open, props.confirmationText] as const,
  () => {
    // Do not carry a destructive confirmation into a later dialog/opening.
...
function onKeydown(event: KeyboardEvent): void {
  ...
  if (typedConfirmationMatches()) emit('confirm');
}

function onConfirmationKeydown(event: KeyboardEvent): void {
  if (event.key !== 'Enter' || props.loading || !typedConfirmationMatches()) return;
  event.preventDefault();
  emit('confirm');
}
...
<template>
  ...
  <Dialog
    ...
    :title="title"
    height="auto"
    :initial-focus="requiresTypedConfirmation() ? '.confirm-dialog__input' : '.confirm-dialog__confirm'"
  ...
  >
    ...
    <label v-if="requiresTypedConfirmation()" class="confirm-dialog__typed-confirmation">
...
.confirm-dialog__typed-confirmation {
  display: grid;
  gap: var(--space-2);
