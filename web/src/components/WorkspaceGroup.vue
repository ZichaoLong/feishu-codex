...
<template>
  <div class="group" :class="{ dragging }">
    <div
      ...
      :draggable="allowWorkspaceReorder !== false"
      @click.stop="emit('groupClick', group.workspace.id, $event)"
      @contextmenu="allowWorkspaceActions !== false && emit('groupContextmenu', group.workspace, $event)"
    ...
    >
      <div class="gh-top">
        ...
        <div
          v-if="renamingId !== group.workspace.id && (allowWorkspaceActions !== false || allowCreate !== false)"
          class="gh-actions"
          :class="{ open: wsMenuOpenId === group.workspace.id }"
        >
          <IconButton
            v-if="allowWorkspaceActions !== false"
            class="gh-more"
            :class="{ open: wsMenuOpenId === group.workspace.id }"
            size="sm"
    ...
    >
      <SessionRow
        ...
        :session-actions="allowSessionActions"
