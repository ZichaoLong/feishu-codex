  ...
  >
    ...
    <div class="col" :style="{ width: colWidth + 'px' }">
      <!-- Header: brand + collapse. The collapse button lives INSIDE the header
           ...
           window-drag strip — there the toggle is the shell's resident floating
           button beside the traffic lights. -->
      <div class="ch">
        <div class="ch-brand">
          <template v-if="!isMacosDesktop">
            <span ref="logoRef" class="ch-logo focus-mark" role="img" aria-label="Focus" @click="onLogoClick">F</span>
            <span class="ch-name">Focus</span>
          </template>
        </div>
        <IconButton
          v-if="!isMacosDesktop"
      ...
      <div v-if="allowCreate || allowWorkspaceCreate" class="btn-wrap">
        <button v-if="allowCreate" class="btn-new-chat" type="button" @click.stop="emit('create')">
          <Icon name="chat-new" />
          <span>{{ t('sidebar.newChat') }}</span>
        </button>
        <IconButton
          v-if="allowWorkspaceCreate && showNewWorkspaceButton"
          size="sm"
        ...
        <template v-else>
          ...
          >
            <WorkspaceGroup
              ...
              :allow-session-actions="allowSessionActions"
              :session-action-capabilities="sessionActionCapabilities"
              :allow-workspace-actions="allowWorkspaceActions"
              :allow-workspace-reorder="allowWorkspaceReorder"
    ...
    <Menu
      v-if="allowWorkspaceActions && ghMenuOpen"
      ref="ghMenuRef"
      class="gh-menu"
      :style="ghMenuStyle"
      @click.stop
    ...
    <Menu
      v-if="allowWorkspaceActions && wsMenuOpenId !== null && wsMenuTarget"
      ref="wsMenuRef"
      class="ws-menu"
      :style="wsMenuStyle"
      @click.stop
    ...
    <Menu
      v-if="allowWorkspaceReorder && sectionMenuOpen"
      ref="sectionMenuRef"
      class="section-menu"
    ...
    <SearchSessionsDialog
      v-if="showSearch"
      :sessions="searchSessions ?? sessions"
      :active-id="activeId"
...
/* Session column. Width is set inline from the shell resize handle; it stays
