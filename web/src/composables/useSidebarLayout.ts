...
// Minimum width of the active Focus detail panel (`.focus-detail`).
...
export function useSidebarLayout(options: UseSidebarLayoutOptions = {}) {
  ...
  // True while the sidebar ResizeHandle is being dragged, so the sidebar can
  // disable its width transition and follow the pointer 1:1.
  ...
  const sidebarMax = computed(() => {
    const reserve = CONVERSATION_MIN + (toValue(options.previewOpen) ? DETAIL_PANEL_MIN : 0);
