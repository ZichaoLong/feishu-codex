...
function detect(): DesktopEnv {
  ...
  // is not applied in every dev-server transform. Fall back to `false` so a
  // plain browser session doesn't throw a ReferenceError on startup; the runtime
  // query-string/sessionStorage signals below still detect a desktop shell.
