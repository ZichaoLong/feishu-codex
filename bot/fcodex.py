...
def _handle_internal_command(
    ...
    *,
    ...
    if cmd in {"/help", "/help-resume", "/help-session"}:
        ...
        print("fcodex /session [cwd|global]  使用 feishu-codex 的共享发现逻辑列出线程。")
        print("fcodex /resume <thread_id|thread_name>  使用共享跨 provider 精确匹配后恢复。")
        print("说明：`/help` 会显示 wrapper 自命令；运行中的 TUI 内输入 `/help` 则是 upstream Codex 帮助。")
        print("说明：运行中的 TUI 内置 /resume 保持 upstream 原样；")
        print("      当前版本通常按 backend 默认 provider 过滤，不受 feishu-codex /profile 控制。")
        print("说明：以上 wrapper 自命令必须单独使用，不能与裸 codex 参数混用。")
        print("说明：除 `/help`、`/profile`、`/rm`、`/session`、`/resume` 外，其余参数与子命令都会继续传给 codex。")
        print("说明：plain `fcodex` / `fcodex <prompt>` 会直接连到 shared backend；目录语义仍由 `--cd` / 当前 shell cwd 决定。")
        print("说明：raw `resume` / `fork` / TUI 内置 `/resume` 仍保持 upstream 原样。")
        return 0
