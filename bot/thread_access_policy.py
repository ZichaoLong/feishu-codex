...
class ThreadAccessPolicy:
    ...
    @staticmethod
    def write_denied_text(owner_label: str) -> str:
    ...
    def interaction_denied_text(cls, lease: InteractionLease | None) -> str:
        ...
        if lease is not None and lease.holder.kind == "feishu":
            owner_label = "另一飞书会话"
        return cls.write_denied_text(owner_label)
    ...
    def prompt_write_denial_text(
        ...
        *,
        ...
        sharing_violation = self.thread_sharing_policy_violation(
            chat_id,
            thread_id,
            message_id=message_id,
