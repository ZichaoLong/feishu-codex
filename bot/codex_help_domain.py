...
class CodexHelpDomain:
    ...
    def _help_overview_text(self) -> str:
        return (
            ...
            "如果当前 turn 仍在执行，私聊可直接继续发消息插播；群聊仅管理员或本轮提问者可插播。\n\n"
            "**命令**\n"
            "- `/new` 立即新建线程\n"
            "- `/session` 查看当前目录线程\n"
    ...
    def _help_session_text(self) -> str:
        return (
            ...
            "- 如果当前 turn 正在执行，继续发送普通文本会优先插播到当前 turn；只有当前 turn 不支持插播时，才会排队到本轮结束后再起下一轮。\n"
    ...
    def _help_group_text(self) -> str:
        return (
            ...
            "- 群聊共享会话正在执行时，只有管理员或本轮提问者可以继续发普通文本插播；若当前 turn 不支持插播，这些消息会排队到本轮结束后继续处理。\n"
