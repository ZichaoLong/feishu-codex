...
class CodexHelpDomain:
    ...
    def _help_overview_text(self) -> str:
        return (
            ...
            "- `/status` 查看当前状态\n\n"
            "- `/release-feishu-runtime` 释放当前线程在 Feishu 侧的 runtime 持有\n\n"
    ...
    def _help_session_text(self) -> str:
        return (
            ...
            f"- `{_SHARED_RESUME_COMMAND.slash_name}` 会在 thread 当前 `notLoaded` 时尝试应用当前默认 profile 的 model 和 model_provider；如果 thread 仍 `loaded`，则不能借 `resume` 改 provider。\n"
            f"- 如果匹配到多个同名线程，`{_SHARED_RESUME_COMMAND.slash_name}` 会报错，不会替你猜。\n"
            "- `/release-feishu-runtime` 会保留当前绑定，但释放 Feishu 自己对这个 thread 的 runtime 持有；如果还有本地 `fcodex` 等外部订阅者，backend 仍可能保持 loaded。\n"
